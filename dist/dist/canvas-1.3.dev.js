...
function () {
  function DrawingCanvas() {
    var _this = this; // Bounding div initialization; for widget bars and canvas
  ...
  DrawingCanvas.prototype.addToolbar = function (side) {
    ...
    var newToolbar = document.createElement('div'); // TODO: flex container for left and right columns
  ...
  DrawingCanvas.prototype.addButton = function (funct, side, img) {
    ...
    var _this = this; // Checks for parameter input errors
    ...
    if (funct === 'clear') newButton.onclick = function () {
      return _this.resetCanvas();
    };else newButton.onclick = function () {
      return _this.setCurrentAction(funct);
    };
    newButton.id = 'drawing-canvas-button' + this._canvasID + '-' + funct;

    if (img != null) {
      var buttonImg_1 = document.createElement('i');
      img.split(' ').forEach(function (cls) {
        return buttonImg_1.classList.add(cls);
  ...
  DrawingCanvas.prototype.addDownloadButton = function (side, img) {
    ...
    if (img != null) {
      var buttonImg_2 = document.createElement('i');
      img.split(' ').forEach(function (cls) {
        return buttonImg_2.classList.add(cls);
      }); // let buttonImg = document.createElement('img');
      // buttonImg.src = img;

      newButton.appendChild(buttonImg_2);
    }
  ...
  DrawingCanvas.prototype.findxy = function (e) {
    ...
    if (e.type === 'mousedown') {
      ...
      this.lastPoint = {
        ...
        color: document.getElementById('drawing-canvas-button-' + this._canvasID + '-color-wheel').value,
        lineWidth: this.lineWidth
      };
      ...
      if (this.currentAction == 'draw') {
        ...
        this.currentDrawing = {
          type: 'line',
          start: JSON.parse(JSON.stringify(this.currentPoint)),
          end: undefined,
          color: color,
          lineWidth: this.lineWidth.valueOf()
        };
        ...
        this.currentDrawing = {
          type: 'circle',
          center: JSON.parse(JSON.stringify(this.currentPoint)),
          radius: undefined,
          color: color,
          lineWidth: this.lineWidth.valueOf()
        };
  ...
  DrawingCanvas.prototype.erase = function () {
    ...
    this.drawings.forEach(function (drawing) {
      if (drawing.type == 'hand_drawn_line') {
        ...
        var A = drawing.start;
        var B = {
          x: drawing.start.x + drawing.width,
          y: drawing.start.y,
          color: drawing.color,
        ...
        var C = {
          x: drawing.start.x + drawing.width,
          y: drawing.start.y + drawing.height,
          color: drawing.color,
          lineWidth: _this.lineWidth
        };
        var D = {
        ...
        if (_this.__distanceBetweenLineAndPoint(_this.currentPoint, A, B) <= drawing.lineWidth || _this.__distanceBetweenLineAndPoint(_this.currentPoint, B, C) <= drawing.lineWidth || _this.__distanceBetweenLineAndPoint(_this.currentPoint, C, D) <= drawing.lineWidth || _this.__distanceBetweenLineAndPoint(_this.currentPoint, D, A) <= drawing.lineWidth) {
          console.log('erase');
          toErase.push(drawing);
        }
    ...
    this.drawings = this.drawings.filter(function (element) {
      return !toErase.includes(element);
    });
    this.redrawCanvas();
